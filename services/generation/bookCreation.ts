...
async function createInRealTime(newBook: BooksAttributes) {
    ...
    textGenerationEmitter.on('textGenerated', (newText: string) => {
        generatedText = newText.replace('\\', '').replace('\n', '').substring(textIndex);

        if (newText === ']}') return;

        try {
            if (bookOverall) {
                [keyValueMap, pageList, textIndex] = createBookInRealTime(newText, keyValueMap, pageList, textIndex);
                if (textIndex !== 0) {
                    bookOverall = false;
                ...
                [pageList, textIndex, currentPageIndex] = createPagesInRealTime(newText, pageList, textIndex, currentPageIndex);
 ...
 */
function createBookInRealTime(newText: string, keyValueMap: Map<string, string | number>, pageList: generatedTextPage[], textIndex: number
): [Map<string, string | number>, generatedTextPage[], number] {

    // Get the keys and values for the book and cover
    let generatedText = newText;
 ...
 */
function createPagesInRealTime(newText: string, pageList: generatedTextPage[], textIndex: number, currentPageIndex: number
